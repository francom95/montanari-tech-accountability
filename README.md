...
**F2.4 — Cuentas bancarias / cuentas de dinero y tarjetas de crédito** (maestros con saldo inicial recalculable):
- ✅ CRUD completo de **CuentaBancaria**: entidad, alias, FK a Moneda, tipo (`CUENTA_CORRIENTE`/`CAJA_AHORRO`/`MERCADO_PAGO`/`CAJA_FISICA`/`OTRA`), estado de conciliación, saldo inicial + fecha, activo. Migración V12 con el seed pedido por la especificación (Banco Galicia CC en ARS, Banco Galicia USD, Mercado Pago).
- ✅ CRUD completo de **TarjetaCredito**: entidad, FK a Moneda, día de cierre/vencimiento, FK a CuentaBancaria de débito, saldo inicial + fecha, activo. Migración V13.
- ✅ **Lógica de saldo inicial** (`common/saldo/`): `CuentaConSaldo` es el contrato mínimo (saldo inicial, fecha, saldo actual) que implementan ambas entidades; `RecalculoSaldoService` es el servicio invocable único que ambos `Service` llaman al crear/editar. Hoy no existe un libro de movimientos (llega en F5.1), así que el saldo actual coincide con el inicial — ese es el punto de extensión que conciliación (F5.x) y flujo de caja (F8.3) van a usar sin tener que tocar `CuentaBancariaService`/`TarjetaCreditoService`.
- ✅ Backend: 14 tests unitarios nuevos (6 CuentaBancaria + 6 TarjetaCredito + 2 de `RecalculoSaldoService`, incluyendo el caso explícito que pide la spec: modificar el saldo inicial y verificar que el saldo actual se recalcula). Frontend: `cuentas-bancarias-page.tsx` y `tarjetas-credito-page.tsx`.
- ✅ Verificado en vivo: el seed de las 3 cuentas existe tras el rebuild, editar el saldo inicial de una cuenta recalcula el saldo actual en la misma respuesta, una tarjeta creada con FK a esa cuenta y a Moneda ARS también recalcula correctamente (incluyendo saldo negativo, el caso típico de una tarjeta de crédito).

A diferencia de F2.1-F2.3, este paso lo ejecutó directamente Sonnet 5 (sin pasar primero por Haiku), a pedido explícito del usuario. El backend compiló limpio a la primera y el stack levantó healthy en el primer intento — sin bugs que documentar esta vez, lo cual es consistente con que los bugs de las rondas anteriores venían del paso Haiku→Sonnet, no de la plantilla en sí.
