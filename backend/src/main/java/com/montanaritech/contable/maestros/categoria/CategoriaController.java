...
public class CategoriaController {
    ...
    @GetMapping
    public Page<CategoriaResponse> listar(@RequestParam(required = false) String texto, @RequestParam(required = false) Boolean activo, Pageable p) {
    ...
    public CategoriaResponse crear(@Valid @RequestBody CategoriaCrearRequest req) {
        return mapper.aResponse(service.crear(req));
    }
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMINISTRADOR', 'CARGA')")
    public CategoriaResponse editar(@PathVariable Long id, @Valid @RequestBody CategoriaEditarRequest req) {
        return mapper.aResponse(service.editar(id, req));
    }
    @PatchMapping("/{id}/activar")
    @PreAuthorize("hasAnyRole('ADMINISTRADOR', 'CARGA')")
    public CategoriaResponse activar(@PathVariable Long id) {
