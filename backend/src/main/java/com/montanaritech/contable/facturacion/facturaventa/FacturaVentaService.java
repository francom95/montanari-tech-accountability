...
public class FacturaVentaService {
     ...
     * uno nuevo vía {@link FacturaVentaAsientoGenerator} — evita duplicar un
     * asiento que ya registra el mismo movimiento económico. El asiento
     * debe existir y estar CONFIRMADO.
     */
    @Transactional
    public FacturaVenta confirmarVinculandoAsientoExistente(Long id, Long asientoId) {
        FacturaVenta f = obtener(id);
        var antes = mapper.aResponse(f);
        TransicionEstadoValidator.validar(f.getEstado(), EstadoDocumento.CONFIRMADO);
