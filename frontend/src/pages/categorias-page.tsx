...
export function CategoriasPage() {
  ...
  const query = useCategorias({ texto, page, size: 10 })
  const crear = useCrearCategoria()
  const editar = useEditarCategoria()
  const cambiarEstado = useCambiarEstadoCategoria()
  const eliminar = useEliminarCategoria()

  const form = useForm<Valores>({ resolver: zodResolver(esquema), defaultValues: { nombre: "", descripcion: "", tipo: "ACTIVO" } })

  function iniciarEdicion(e: Categoria) {
    setEditando(e)
    form.reset({ nombre: e.nombre, descripcion: e.descripcion ?? "", tipo: e.tipo as Valores["tipo"] })
  }

  function cancelarEdicion() {
    setEditando(null)
    form.reset({ nombre: "", descripcion: "", tipo: "ACTIVO" })
  }
  ...
  function onSubmit(valores: Valores) {
    if (editando) {
      ...
      crear.mutate(valores, { onSuccess: () => form.reset({ nombre: "", descripcion: "", tipo: "ACTIVO" }) })
    }
  }

  const columnas = useMemo<ColumnDef<Categoria>[]>(
    () => [
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-lg font-semibold text-foreground">Categorías contables</h1>
        <p className="text-sm text-muted-foreground">Molde PL-1/PL-2 replicado de Moneda (F2.1).</p>
      </div>

      <Card>
        <CardHeader><CardTitle>{editando ? `Editar ${editando.nombre}` : "Nueva categoría"}</CardTitle></CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4 sm:grid-cols-4">
              <FormField control={form.control} name="nombre" render={({ field }) => (
                <FormItem><FormLabel>Nombre</FormLabel><FormControl><Input {...field} placeholder="Activo Corriente" /></FormControl><FormMessage /></FormItem>
              )} />
