...
import { descargarCuentasPorPagarExcel, descargarCuentasPorPagarPdf, useCuentasPorPagar } from "@/hooks/use-cuenta-por-pagar"
...
import type { CuentaPorPagarFiltros, EstadoVencimiento } from "@/types/cuenta-por-pagar"
...
export function CuentasPorPagarPage() {
  ...
  const filtros: CuentaPorPagarFiltros = useMemo(() => ({
  ...
  }), [proveedorId, proyectoId, monedaId, fechaDesde, fechaHasta, estadoVencimiento])

  const reporte = useCuentasPorPagar(filtros)
  ...
  async function exportar(formato: "excel" | "pdf") {
    ...
    try {
      if (formato === "excel") await descargarCuentasPorPagarExcel(filtros)
      else await descargarCuentasPorPagarPdf(filtros)
    } finally {
      setDescargando(null)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-foreground">Cuentas por pagar</h1>
          <p className="text-sm text-muted-foreground">Facturas de compra confirmadas con saldo pendiente (F4.4: no recalcula saldos).</p>
        </div>
      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap items-end gap-2">
          ...
          <label className="flex flex-col text-xs text-muted-foreground">
            Proveedor
            <select value={proveedorId} onChange={(e) => setProveedorId(e.target.value)} className={`${selectClase} min-w-40`} disabled={proveedores.isLoading}>
                ...
                <table className="w-full min-w-[900px] text-left text-sm">
                  ...
                  <tbody>
                    {(reporte.data?.filas ?? []).map((f) => (
                      <tr key={f.facturaCompraId} className="border-b border-border last:border-0">
                        <td className="py-2 pr-4">{f.proveedorNombre}</td>
